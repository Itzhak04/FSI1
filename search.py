...
def graph_search(problem, fringe):
    ...
    closed = {}
    result = Results(None, 0, 1, 0)
    result.timing = timeit.timeit()
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            result.node = node
            result.timing = result.timing-timeit.timeit()
            return result
            #return node
        if node.state not in closed:
            closed[node.state] = True
            sucesores = node.expand(problem)
            result.generatedNode += len(sucesores)
            fringe.extend(sucesores)
    return None
...
def subestimacion_search(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""
    return graph_search(problem, PriorityQueue2(problem))
...
class Results:
    ...
    @property
    def visit_nodeg(self):
        return self.visitNode

    @visit_nodeg.setter
    def visitNodes(self, visitNode):
        self.visitNode = visitNode

    @property
    def generatedNodesg(self):
        return self.generatedNode;
    @generatedNodesg.setter
    def generatedNodes(self, generatdNode):
        self.generatedNode = generatdNode
